...
class TestStrategy(bt.Strategy): # copied from Bactrader quick start guide
    ...
    def __init__(self):
        ...
        # self.dummy1=DummyInd(self.datas[0])
    ...
    def next(self):
        ...
        if not self.position and self.notFin:
            ...
            if len(self) >= (self.bar_executed + 2):
                ...
                # Keep track of the created order to avoid a 2nd order
                self.order = self.sell(data=self.datas[2])
                self.notFin=False
#
...
tv=TvDatafeed()
data3m=tv.get_hist("ETHUSDT", "KUCOIN", n_bars=20, interval=Interval.in_3_minute)
data5m=tv.get_hist("ETHUSDT", "KUCOIN", n_bars=20, interval=Interval.in_5_minute)
data15m=tv.get_hist("ETHUSDT", "BINANCE", n_bars=20, interval=Interval.in_3_minute)
